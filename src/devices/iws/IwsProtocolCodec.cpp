                                   ...
                                   const QList<quint16>& parameters)
{
    ...
    // (запомнить запрошенные регистры — забота вызывающей стороны:
    //  это её состояние, см. GroundMeteoParams::createModbusReadRequest)
...
{
    if (response.size() < 12) {
        errorMsg = "UMB response too short";
        return false;
    }

    if (response[0] != 0x01 || response[1] != 0x10 ||
        response[response.size()-1] != 0x04) {
        errorMsg = "Invalid UMB packet structure";
        return false;
    }
    ...
    if (stxPos >= response.size() || response[stxPos] != 0x02) {
        errorMsg = "STX not found at expected position";
        return false;
    }

    int etxPos = stxPos + 1 + dataLength;
    if (etxPos >= response.size() || response[etxPos] != 0x03) {
        errorMsg = "ETX not found at expected position";
        return false;
    }
    ...
    if (expectedCrc != receivedCrc) {
        errorMsg = QString("CRC mismatch: expected 0x%1, got 0x%2")
    ...
    if (dataPos + 3 >= etxPos) {
        errorMsg = "Invalid data section in UMB response";
        return false;
    }
