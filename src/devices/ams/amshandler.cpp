...
// ── Запись в архив ──────────────────────────────────────────────────────────
// Все запросы к базе переехали в data/ArchiveWriter: драйвер линии RS-485 не
// должен заодно быть слоем хранения. Здесь остались тонкие обёртки — они
// подставляют номер текущей записи архива (состояние драйвера) и эмитят
// databaseError(), которого у свободных функций записи быть не может.
// Публичный интерфейс класса не изменился, поэтому вызывающий код прежний.
...
{
    return ArchiveWriter::saveMeteo11Bulletin(m_currentRecordId, bulletinJson,
                                              bulletinTime, validityPeriod);
}

int AMSHandler::createMainArchiveRecord(const QString &notes)
{
    QString errorMsg;
    const int recordId = ArchiveWriter::createMainArchiveRecord(notes, errorMsg);
    if (!errorMsg.isEmpty())
...
{
    return ArchiveWriter::deleteCalculatedWindProfiles(recordId);
}

bool AMSHandler::saveAvgWindProfile(int recordId, const QVector<WindProfileData> &data)
{
    QString errorMsg;
    const bool ok = ArchiveWriter::saveAvgWindProfile(recordId, data, errorMsg);
    if (!errorMsg.isEmpty())
        emit databaseError(errorMsg);
    return ok;
}
...
{
    QString errorMsg;
    const bool ok = ArchiveWriter::saveActualWindProfile(recordId, data, errorMsg);
    if (!errorMsg.isEmpty())
        emit databaseError(errorMsg);
    return ok;
}
...
{
    QString errorMsg;
    const bool ok = ArchiveWriter::saveMeasuredWindProfile(recordId, data, errorMsg);
    if (!errorMsg.isEmpty())
        emit databaseError(errorMsg);
    return ok;
}
...
{
    QString errorMsg;
    const bool ok = ArchiveWriter::saveStationCoordinates(recordId, coords, errorMsg);
    if (!errorMsg.isEmpty())
        emit databaseError(errorMsg);
    return ok;
}

bool AMSHandler::saveCriticalMessage(int recordId, const QString &message,
                                     const QString &severity)
{
    QString errorMsg;
    const bool ok = ArchiveWriter::saveCriticalMessage(recordId, message, severity, errorMsg);
    if (!errorMsg.isEmpty())
        emit databaseError(errorMsg);
    return ok;
}
