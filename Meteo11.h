...
private:
    ...
    // Заполняет "//" (код отсутствующих данных) в НН/СС/ПП всех строк таблицы
    // после последней строки, где оператор реально ввёл НН или СС — так же,
    // как в архивных бюллетенях отмечаются высоты, до которых зонд не долетел.
    void fillMissingTrailingLayers();
