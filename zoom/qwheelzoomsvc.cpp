    ...
    QObject()
{
    ...
    // ИСПРАВЛЕНИЕ: уменьшен с 1.2 до 1.1 для более плавного масштабирования
    sfact = 1.1;
}
...
bool QWheelZoomSvc::eventFilter(QObject *target,QEvent *event)
{
    ...
    if (target == zoom->plot())
    {
        ...
        if (event->type() == QEvent::Wheel)
        {
            ...
            // ИСПРАВЛЕНИЕ: replot() уже вызывается внутри set(), убираем дублирование
    ...
    if (wEvent->orientation() == Qt::Vertical)
    {
        ...
        if (wd != 0)    // если колесо вращалось, то
        {
            ...
            if (ax) // если задано масштабирование по горизонтали
            {
                ...
                // ИСПРАВЛЕНИЕ: сохраняем исходную полуширину для проверки
                double original_dx = dx;
                ...
                // ИСПРАВЛЕНИЕ: ограничение минимальной ширины (не менее 1% от исходной)
                double min_dx = zoom->isb_x->max - zoom->isb_x->min;
                min_dx = min_dx * 0.01;  // минимум 1% от полного диапазона
                if (dx < min_dx) dx = min_dx;
                
                // ИСПРАВЛЕНИЕ: ограничение максимальной ширины (не более 200% от исходной)
                double max_dx = (zoom->isb_x->max - zoom->isb_x->min);
            ...
            if (ay) // если задано масштабирование по вертикали
            {
                ...
                // ИСПРАВЛЕНИЕ: ограничение минимальной высоты (не менее 1% от исходной)
                double min_dy = zoom->isb_y->max - zoom->isb_y->min;
                min_dy = min_dy * 0.01;  // минимум 1% от полного диапазона
                if (dy < min_dy) dy = min_dy;
                
                // ИСПРАВЛЕНИЕ: ограничение максимальной высоты (не более 200% от исходной)
                double max_dy = (zoom->isb_y->max - zoom->isb_y->min);
