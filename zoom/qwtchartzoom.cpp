...
bool QwtChartZoom::updateVerAxis(int yAx,int *vDiv)
{
    ...
    int dv = floor(qwtp->canvas()->size().height()/(szlb.height()+30));
...
// Конструктор
QScaleBounds::QScaleBounds(QwtPlot *plt,QwtPlot::Axis mst,QwtPlot::Axis slv) : QObject() {
    // запоминаем
...
void QScaleBounds::set(double mn,double mx)
{
    ...
    emit axisScaleChanged(master, mn, mx);
                                        // и дополнительной
    if (sync) {
        plot->setAxisScale(slave,mn,mx);
        emit axisScaleChanged(slave, mn, mx);
    }
    else {
...
void QScaleBounds::dup()
{
    ...
    if (sync) {
        plot->setAxisScale(slave,sm.s1(),sm.s2());
        emit axisScaleChanged(slave, sm.s1(), sm.s2());
    }
    else {
        plot->setAxisScale(slave,ak*sm.s1()+bk,ak*sm.s2()+bk);
        emit axisScaleChanged(slave, ak*sm.s1()+bk, ak*sm.s2()+bk);
    }
}
